resource "aws_cloudwatch_metric_alarm" "burst_balance_too_low" {
  alarm_name          = "${var.prefix}efs-${var.file_system_id}-lowBurstBalance"
...
resource "aws_cloudwatch_metric_alarm" "io_percentage_too_high" {
  alarm_name          = "${var.prefix}efs-${var.file_system_id}-highIOPercentage"
...
resource "aws_cloudwatch_metric_alarm" "anomalous_client_connections" {
  alarm_name          = "${var.prefix}efs-${var.file_system_id}-anomalousClientConnections"
...
resource "aws_cloudwatch_metric_alarm" "anomalous_io_bytes" {
  alarm_name          = "${var.prefix}efs-${var.file_system_id}-anomalousIOBytes"
  comparison_operator = "GreaterThanUpperThreshold"
  evaluation_periods  = var.evaluation_period
  threshold_metric_id = "e1"
